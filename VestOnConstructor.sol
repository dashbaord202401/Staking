...
contract TokenVesting is ERC20, ReentrancyGuard {
    ...
    ) ERC20("Ommniverse", "OMMI") {
        _mint(address(this), 100000 * 10**decimals());
        uint256 arrayLen = _beneficiary.length;
        require(
            arrayLen == _start.length &&
                arrayLen == _cliff.length &&
                arrayLen == _duration.length &&
                arrayLen == _slicePeriodSeconds.length &&
                arrayLen == _amount.length,
            "Wrong input"
        );

        for (uint256 i = 0; i < arrayLen; i++) {
            require(
                _beneficiary[i] != address(0),
                "beneficiary addr cann't be 0"
            );
            require(_duration[i] > 0, "TokenVesting: duration must be > 0");
            ...
            uint256 cliff = _start[i].add(_cliff[i]);
            vestingSchedules[_beneficiary[i]] = VestingSchedule(
                _beneficiary[i],
                _start[i],
                cliff,
                _duration[i],
            ...
            vestingBeneficiaries.push(_beneficiary[i]);
        }
    }
    ...
    function release() public nonReentrant {
        ...
        transfer(beneficiaryPayable, vestedAmount);
    }
